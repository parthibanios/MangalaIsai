...
public class DTTextField: UITextField {
        ...
        didSet{
            ...
            case .rounded:
                if UIDevice.current.userInterfaceIdiom == .phone {
                    dtLayer.cornerRadius        = 20
                }
                else
                {
                     dtLayer.cornerRadius        = 30
                }
               
                dtLayer.borderWidth         = 1
    ...
    public var errorFont = (UIDevice.current.userInterfaceIdiom == .phone ? UIFont.systemFont(ofSize: 12) : UIFont.systemFont(ofSize: 16)){
        didSet{ invalidateIntrinsicContentSize() }
    }
    
    public var floatPlaceholderFont = (UIDevice.current.userInterfaceIdiom == .phone ? UIFont.systemFont(ofSize: 12) : UIFont.systemFont(ofSize: 16)){
        didSet{ invalidateIntrinsicContentSize() }
    }
    ...
    public var borderColor:UIColor = UIColor.appColorPink
    {
    ...
    fileprivate var dtLayerHeight:CGFloat{
        return bounds.height//showErrorLabel ? floor(bounds.height - lblError.bounds.size.height - paddingYErrorLabel) : bounds.height
    }
    ...
    @IBInspectable var leftImage : UIImage? {
        didSet {
            if let image = leftImage{
                ...
                var view = UIView()
                if UIDevice.current.userInterfaceIdiom == .phone {
                    imageView = UIImageView(frame: CGRect(x: 10, y: (self.frame.size.height-11)/3, width: (self.frame.size.height-21) , height: (self.frame.size.height - 21) ))
                     view = UIView(frame : CGRect(x: 5, y: 0, width: self.frame.size.height, height: self.frame.size.height ))
                }
                else
                {
                    imageView = UIImageView(frame: CGRect(x: 10, y: ((self.frame.size.height)/3), width: (self.frame.size.height-10) , height: (self.frame.size.height-10) ))
                     view = UIView(frame : CGRect(x: 5, y: 0, width: self.frame.size.height + 10 , height: self.frame.size.height ))
